...
public:
    ...
    struct SdoCatch : sc::state<SdoCatch, RoboSimMachine> {
        ...
        SdoCatch(my_context ctx) : my_base(ctx) {
            const auto &ball_pos = context<RoboSimMachine>().agent->world().ball().pos();
    ...
    struct DoMove : sc::state<DoMove, RoboSimMachine> {
        ...
        DoMove(my_context ctx) : my_base(ctx) {
            const auto &ball = context<RoboSimMachine>().agent->world().ball().pos();
            const auto &goalPos = context<RoboSimMachine>().agent->world().ourGoal().center();
